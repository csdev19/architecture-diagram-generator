...
📖 **See the complete architecture with all layers in:** [Bounded Contexts Complete Guide](/architecture/bounded-contexts-complete-guide)
...
Durable architectural decisions, with the context that produced them and the
condition that would reopen them.

- [ADR 0001 — No canvas library for the editor](/architecture/decisions/adr-0001-no-canvas-library-for-the-editor)
  — why `/editor` keeps a hand-written SVG canvas instead of adopting React Flow.
