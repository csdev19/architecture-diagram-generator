import { createServerFn } from "@tanstack/react-start";
import type { ApiResponse } from "@diagram-tool/domain/types";
import type { TodoBase, UpdateTodo } from "@diagram-tool/domain/schemas";
import { updateTodoSchema } from "@diagram-tool/domain/schemas";
import { createDatabaseClient } from "@diagram-tool/infra-db/client";
import { TodoRepository } from "@diagram-tool/infra-db/repositories";
import { getAuthSession } from "@/lib/auth/get-auth-session";
import { env } from "@/env/server";
import z from "zod";

const updateTodoInputSchema = z.object({
  id: z.string(),
  data: updateTodoSchema,
});

export const updateTodo = createServerFn({ method: "POST" })
  .inputValidator((input: { id: string; data: UpdateTodo }) => updateTodoInputSchema.parse(input))
  .handler(async (ctx): Promise<ApiResponse<TodoBase>> => {
    const session = await getAuthSession();

    if (!session) {
      return { data: null, error: { message: "Unauthorized" } };
    }

    const db = createDatabaseClient(env.DATABASE_URL);
    const repo = new TodoRepository(db);
    const todo = await repo.update(ctx.data.id, session.user.id, ctx.data.data);

    if (!todo) {
      return { data: null, error: { message: "Todo not found" } };
    }

    return { data: todo, error: null };
  });
